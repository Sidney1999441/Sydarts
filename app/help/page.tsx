...
const quickStart = [
  ...
  {
    title: "计分或录入结果",
    text: "硬式比赛可用系统计分器；软式比赛当前手动录入。双人/队制里的单人局，需要保存本局实际上场选手。"
  },
  {
    title: "赛后看统计",
...
const workflows = [
  ...
  },
  {
    ...
    steps: [
      "软式比赛当前不进入实时计分器。",
      "比赛结束后进入赛事页面或后台成绩页手动录入。",
      "选择胜方、比分、每局上场名单。",
      "按选手填写平均分、平均 MPR、TON80、帽子戏法、高拆、白马等。",
...
const featureGroups: Array<{ title: string; icon: IconType; items: string[]; checks: string[] }> = [
  ...
  },
  {
    ...
    items: [
      ...
      "软式目前手动录入，同时预留软镖机接入 API。"
    ],
    checks: ["硬式比赛可进入计分器。", "软式比赛列表提示手动录入。", "软式个人数据写入软式统计。"]
  },
  {
    title: "计分器",
    icon: Gauge,
...
const qa = [
  ...
  {
    q: "为什么软式不能打开计分器？",
    a: "当前软式只支持手动录入，因为软镖机实时数据还未接入。系统已经保留接入 API，后续可对接软镖机平台。"
  },
  {
    q: "雪分制为什么只能双人？",
