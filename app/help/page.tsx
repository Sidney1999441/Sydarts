...
const workflows = [
  ...
  },
  {
    ...
    steps: [
      ...
      "按回合录入分数，计分区显示当前局 PPR，结账成功后自动切到下一局。",
      "保存进度会一并保留撤销历史；恢复计分后，可以逐次撤销此前录错的回合。旧版未保存的撤销历史无法补回。",
      "若赛事设置为领先过半，达到胜局后自动结束；若设置为打满，则继续完成全部局。",
      "比赛结束后可查看每名选手每一局的 PPR、最高轮、180 和镖数；保存后写入比赛历史、个人统计和段位计算。"
    ]
  },
  {
    title: "赛事现场：软式比赛",
    ...
    steps: [
      ...
      "录入过程会自动保存到服务器和当前设备；断网时保留本机进度，重新联网或再次进入后自动恢复。",
      "展开已录局次可修改任意已录局的成绩，后续局次保留，总比分自动重算；正式结算后由管理员更正。",
      "选择胜方、比分、每局上场名单。",
      "01 项目可直接填 PPR，也可填 PPD 自动换算；米老鼠填写小于 10 的 MPR、Mark 和白马；高分赛只填写本局得分与可选特殊数据。",
...
const featureGroups: Array<{ title: string; icon: IconType; items: string[]; checks: string[] }> = [
  {
    ...
    items: [
      ...
      "用户资料包含显示名、头像、手机号、简介、角色、状态和多套等级基准。",
  ...
  },
  {
    ...
    items: [
      ...
      "预约日历可按日期、机台查看空闲与已预约时段，所有时间以北京时间显示。",
      "完成本周比赛不会隐藏未完成补赛；补赛会持续标黄保留到完赛。"
    ],
  ...
  },
  {
    ...
    icon: Star,
    items: [
      "赛事排行榜顶部显示最近一周赛事之星，并按赛事第 1 周、第 2 周等周次标注；往期获选记录可折叠查看。",
      "系统只读取已结束且有个人数据的正式比赛，并以个人单局最高 PPR/MPR 综合胜负、出场与高光数据。",
      "个人榜单会标注双人局来源；同分时单人局排在双人局之前，累计榜会注明是否包含双人局数据。",
      "管理员可以在赛事后台按周人工调整获选选手和理由，也可以恢复系统自动评选。"
    ],
    checks: ["当前周不会提前评选。", "软硬镖指标先归一化再比较。", "人工调整会明确标注，不会改动原始比赛数据。"]
  },
  {
    title: "计分器",
    icon: Gauge,
  ...
  },
  {
    ...
    items: [
      "手动录入支持胜负、比分、每局出场名单。",
      "软式支持平均分、MPR、TON80、帽子戏法、高拆、白马；MPR 必须小于 10。",
      "成绩需要归属到实际出场个人。"
    ],
    checks: ["双人/队制里的单人局必须选择上场选手。", "比分与胜方必须一致。", "管理员修正已完赛数据后会撤销旧统计并重新结算。"]
  },
  {
    title: "CODL 视觉与触摸体验",
    icon: Palette,
...
const qa = [
  ...
  {
    q: "本周赛程怎么决定？",
    a: "首页按联赛周次展示本周比赛，并独立保留所有未完成补赛。完成本周比赛不会让补赛消失，也不会提前拉入下周比赛。跨周后已完成赛果退出本周列表，未完成补赛继续标黄保留。"
  },
  {
    q: "雪分制为什么只能双人？",
