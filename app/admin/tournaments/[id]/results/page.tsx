  ...
  return (
    <div className="grid gap-6">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-600">管理员可修改结果并处理待确认/争议比赛。</p>
      </div>
      <Card>
        <div className="grid gap-4">
          ...
          {(matches || []).length === 0 ? <p className="text-sm text-slate-600">暂无比赛。</p> : null}
        </div>
      </Card>
      <Card>
        ...
        <div className="mt-4 grid gap-2 text-sm">
          ...
          {(confirmations || []).length === 0 ? <p className="text-slate-600">暂无确认记录。</p> : null}
        </div>
      </Card>
    </div>
  );
